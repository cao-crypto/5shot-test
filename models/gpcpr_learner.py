...
class GPCPRLearner(object):
    ...
    def __init__(self, args, mode='train', logger=None):
        ...
        # Set logger on model if available
        if hasattr(self.model, 'set_logger'):
            self.model.set_logger(logger)
        
        # Log model info to file only
        ...
        if mode == 'train':
            ...
            # Log parameter counts for major modules
            ...
            # Log backbone configuration (file only)
    ...
    def debug_log(self, msg):
        ...
        # Log text embedding loading (file only)
        self.debug_log("load {} {}".format(args.dataset, args.embedding_type))
